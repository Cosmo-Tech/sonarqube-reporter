...
class ReportGenerator:
    ...
    def generate_quality_gate_report(self, projects_data_tuple):
        ...
        css_source = os.path.join(
            os.path.dirname(self.env.loader.searchpath[0]),
            "templates",
            "css",
            "report_styles.css",
        )
    ...
    def _format_date(self, date_string):
        ...
        if not date_string or date_string == "N/A":
            logger.debug("Empty or N/A date string, returning N/A")
            return "N/A"
        ...
        try:
            ...
            logger.debug(
                f"Formatted date for display: {date_string} -> {formatted_date}"
            )
